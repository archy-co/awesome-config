...
local function worker(user_args)
    ...
    --local font = args.font or 'Material-Design-Iconic-Font 10'
    local font = args.font or 'Font Awesome 6 Free 10'
    ...
    local function update_widget(widget, stdout)
        ...
        if status == 'Charging' then
            ...
            text_with_background.fg = '#dddddd'
        else
    ...
    local battery_popup = awful.popup{
        ...
        -- add paddings inside the popup and center content of popup
        margins = {
            top = 10,
            bottom = 10,
            left = 10,
    ...
    local bat_popup_rows  = { layout = wibox.layout.fixed.vertical }

    last_battery_check = 0
    -- Popup with battery info
    local function show_battery_status()
        -- save the last time the popup was shown
        last_battery_check = os.time()

        awful.spawn.easy_async([[bash -c 'acpi']],
                function(stdout, _, _, _)
                    -- Cut Battery0 from the beggining of stdout. Save result back to stdout
                    stdout = string.gsub(stdout, 'Battery 0: ', '')
                    stdout = stdout
                    bat_popup_rows[1] = wibox.widget{
                        {
                            markup = "<b>" .. stdout .. "</b>",
                            align = 'center',
                            widget = wibox.widget.textbox
                    ...
                    battery_popup:setup(bat_popup_rows)
                end)
    ...
    -- check if more then 5 seconds have passed since last_battery_check
    -- if so, show hide the popup if it is visible
    gears.timer {
        timeout   = 3,
        autostart = true,
        callback  = function()
            if os.difftime(os.time(), last_battery_check) > 5 then
                battery_popup.visible = false
            end
        end
    }
