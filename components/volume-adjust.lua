...
local volume_adjust = wibox({
   ...
   y = (screen.geometry.height / 2) - (offsety / 2) + 300,
   width = dpi(48),
   height = offsety,
...
awesome.connect_signal("volume_change",
   function()
      ...
      awful.spawn.easy_async_with_shell(
         -- "amixer sget Master | grep 'Mono: Play' | sed 's/.*off.*/0'/g | sed -r 's/.*\\[([0-9]+)%.*/\\1/g'",
         -- "(pactl get-sink-mute @DEFAULT_SINK@ ; pactl get-sink-volume @DEFAULT_SINK@) | sed -r 's/.*balance.*//g ; s/Mute.*yes/0/g ; s/Mute.*no//g ; s/[^0-9].*\\/[^0-9].* ([0-9]+)%.*/\\1/g'",
         "(pactl get-sink-mute @DEFAULT_SINK@ ; pactl get-sink-volume @DEFAULT_SINK@) | sed -n 'N ; l 100' | sed -r 's/.*balance.*//g ; s/.*Mute.*yes.*/0/g ; s/Mute.*no//g ; s/[^0-9].*\\/[^0-9].* ([0-9]+)%.*/\\1/g'",
         function(stdout)
